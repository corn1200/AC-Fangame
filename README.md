...
- 자세 안정 성능 :
  ...
  - 이 수치가 클수록 쉽게 스태거에 걸리지 않는다
- 시스템 복원 성능
  - 시스템 이상을 감지하고 복원하는 성능
  - 이 수치가 클수록 쉽게 시스템 이상에 걸리지 않는다
- 스캔 거리
  - 스캔으로 적이나 파츠 컨테이너를 검출할 수 있는 거리
- 스캔 지속 시간
  - 스캔으로 적이나 파츠 컨테이너를 검출했을 때의 지속 시간
- 스캔 대기 시간
  - 스캔 사용 후 다시 사용할 수 있게 되는 대기 시간
...
- 부스터 효율 보정 :
  - 부스터의 EN 효율에 대한 보정
  - 이 수치가 클수록 퀵 부스트의 소비 EN이 적어진다
- 제너레이터 출력 보정 :
  - 제너레이터의 EN 출력에 대한 보정
  - 이 수치가 클수록 최종 출력이 향상된다
- 제너레이터 공급 보정 :
  - 제너레이터의 EN 공급 성능에 대한 보정
  - 이 수치가 클수록 EN 소비 액션을 사용한 뒤 공급이 재개되는 속도가 빨라진다
...
- 반동 제어 :
  - 사격 반동을 흡수하는 성능
  - 이 수치가 클수록 연사로 인한 사격 정밀도 저하가 쉽게 발생하지 않는다
- 사격 무기 적성 :
  - 사격 무기 사용에 대한 적성
  - 이 수치가 클수록 조준 추적 성능이 향상된다
- 근접 무기 적성 :
  - 근접 무기 사용에 대한 적성
  - 이 수치가 클수록 위력이 향상된다
...
- 적재 상한 :
  - 다리가 지탱할 수 있는 중량 상한
  - 이 수치가 클수록 중량 파츠를 다수 탑재할 수 있다
- 수평 도약 성능 :
  - 수평 방향 도약 성능
  - 이 수치가 클수록 지상 퀵 부스트의 처음 속도가 빨라진다
- 수직 도약 성능 :
  - 수직 방향 도약 성능
  - 이 수치가 클수록 지상 점프의 도달 고도가 높아진다
...
- 추진력 :
  - 부스트 이동의 추진력
  - 이 수치가 클수록 고속으로 이동할 수 있다
- 상승 추진력 :
  - 부스트 상승의 추진력
  - 이 수치가 클수록 고속으로 상승할 수 있다
- 상승 소비 EN :
  - 부스트 상승으로 소비하는 EN
- QB 추진력 :
  - 퀵 부스트의 초기 추진력
  - 이 수치가 클수록 움직임이 더 재빨라진다
- QB 분사 시간 :
  - 퀵 부스트의 분사 시간
  - 이 수치가 클수록 이동 거리가 더 늘어난다
- QB 소비 EN :
  - 퀵 부스트로 소비하는 EN
- QB 재사용 시간 :
  - 퀵 부스트 리로드 시간
  - 이 수치가 작을수록 연속으로 발동할 수 있다
- QB 재사용 보증 중량 :
  - 퀵 부스트의 재사용 시간이 보장되는 중량
  - 이 수치가 클수록 중량이 늘어나도 성능이 떨어지지 않는다
- AB 추진력 :
  - 어설트 부스트의 추진력
  - 이 수치가 클수록 고속으로 돌격할 수 있다
- AB 소비 EN :
  - 어설트 부스트로 소비하는 EN
...
- QB 재사용 보증 중량 :
  - 퀵 부스트의 재사용 시간이 보장되는 중량
  - 이 수치가 클수록 중량이 늘어난도 성능이 떨어지지 않는다
- AB 추진력 :
  - 어설트 부스트의 추진력
  - 이 수치가 클수록 고속으로 돌격할 수 있다
- AB 소비 EN :
  - 어설트 부스트로 소비하는 EN
- 근접 공격 추진력 :
  - 근접 무기로 적을 추격할 때 발휘되는 추진력
  - 대부분의 근접 무기는 일정 거리 내의 적을 추격할 수 있으며 이 수치가 클수록 추격 속도가 빨라진다
- 근접 공격 소비 EN :
  - 근접 무기 사용 시 이동으로 소비되는 EN
- 중량 :
  - 파츠의 중량
- EN 부하 :
  - 파츠의 EN 부하
...
- 근거리 어시스트 적성 :
  - 근거리 조준 어시스트 적성
  - 이 수치가 클수록 근거리(130m 이내)에서 조준이 쉬워진다
- 중거리 어시스트 적성 :
  - 중거리 조준 어시스트 적성
  - 이 수치가 클수록 중거리(130m~260m 사이)에서 조준이 쉬워진다
- 원거리 어시스트 적성 :
  - 원거리 조준 어시스트 적성
  - 이 수치가 클수록 원거리(260m 너머)에서 조준이 쉬워진다
- 미사일 조준 보정 :
  - 미사일 등 유도탄의 조준 완료 시간에 영향을 주는 보정치
  - 이 수치가 클수록 조준 완료에 걸리는 시간이 짧아진다
- 다중 조준 보정 :
  - 미사일 등 유도탄의 다중 조준 완료 시간에 영향을 주는 보정치
  - 이 수치가 클수록 조준 완료에 걸리는 시간이 짧아진다
- 중량 :
  - 파츠의 중량
- EN 부하 :
...
1. 플레이어, 몹, 오브젝트의 클래스 정의
   ...
   4. 프레임 파츠, 내장 파츠 인터페이스 상세 작성 (2024-04-16)
2. 플레이어의 애니메이션 수집
3. 플레이어 이동(걷기, 부스트, 퀵 부스트, 어설트 부스트, 점프, 활강) 기능 구현
4. 몹이 플레이어 앞뒤에 있는지 판별하는 기능 구현
5. 몹이 풀레이어 시야 범위에 있는지 판별하는 기능 구현
6. 몹이 플레이어 좌우에 있는지 판별하는 기능 구현
7. 절두체 컬링으로 몹 최적화하기